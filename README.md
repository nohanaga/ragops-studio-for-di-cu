...
# USER_TABS_ENABLED=false      # set true to show user tabs
...
- Preview `2026-06-01-preview` is always selectable in the UI. Preview requests never fall back to GA automatically.
...
Azure Developer CLI (`azd`) can provision the required Azure resources, remotely build the container image, and deploy it to Azure Container Apps with `azd up`. Guidance is also available for Azure CLI scripts, Blob / SMB storage, Managed Identity, Container Apps settings, and Easy Auth.

See the **[deployment guide](DEPLOYMENT_GUIDE.md)** for details.
